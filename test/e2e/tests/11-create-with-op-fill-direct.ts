...
/**
 * Test 11: Create another sell order + Taker fills directly
 * Tests the same direct flow with different amounts.
 */
export default async function testCreateAndFillSellOrderVariant(): Promise<boolean> {
  divider("Test 11: Create Sell Order (Variant) + Taker Fills Directly");

  try {
    // Step 1: Build sell order with different amounts
    ...
    const { order, optionTokenId, extensionEncoded } =
      orderHelper.buildSellOptionsOrder({
        ...
        option: TEST_OPTION,
        optionAmount: parseEther("3").toString(), // 3 options
        stableAmount: "45000000", // 45 USDC (15 USDC per option)
      });
    const orderStruct = order.build();

    // Step 2: Sign with MAKER wallet directly
    ...
    const storedOrder = await orderbookApi.createOrder({
      ...
      extensionEncoded,
      signature,
      optionAmount: parseEther("3").toString(),
      stableAmount: "45000000",
      stableToken: CONFIG.usdcAddress,
      makerProxyAddress: CONFIG.erc6909ProxyAddress,
    });
    ...
    const fillAmount = parseEther("1.5"); // Fill 1.5 options (half the order)
