...
/**
 * Test 12: Create sell order (direct) + Fill (direct)
 * Standard direct flow with maker-signed order and taker-filled execution.
 */
export default async function testCreateAndFillBothDirect(): Promise<boolean> {
  divider("Test 12: Create Sell Order + Fill Both Direct");

  try {
    // Step 1: Build order - maker is Account 1
    ...
    // Step 4: Fill DIRECTLY with taker (Account 2)
    ...
    if (optionsReceived > 0n) {
      success(
        `Order filled! Taker received ${formatEther(optionsReceived)} options`
      );
      return true;
    } else {
